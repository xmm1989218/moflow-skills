...
Only `.js` script files are supported. Dependencies are managed via `package.json` and installed with `bun install`:

```bash
cd skills/<name>/scripts && bun install
```

This generates `bun.lockb` which must be committed to git.
